...
import {
  Users, Building2, Waves, Clock, UserCog, GraduationCap,
  Trophy, CalendarRange, Baby, TrendingUp, Activity,
} from "lucide-react";
...
function MiniChart() {
  ...
  return (
    <div className="flex items-end gap-1.5 h-32">
      {bars.map((b, i) => (
        <div key={i} className="flex-1 rounded-t-md bg-linear-to-t from-primary/30 to-primary" style={{ height: `${(b / max) * 100}%` }} />
      ))}
    </div>
  );
}
      ...
      {items.map((it, i) => (
        <li key={i} className="flex items-center gap-3">
          ...
          <div className="flex-1 text-sm">
            <span className="font-medium">{it.who}</span> <span className="text-muted-foreground">{it.what}</span>
          </div>
      ...
      {t.map((x) => (
        <li key={x.name} className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="text-xs text-muted-foreground">{x.date} · {x.reg} registered</div>
          </div>
    ...
    <ul className="space-y-3">
      {e.map((x) => (
        <li key={x.swimmer} className="flex items-center gap-3">
          ...
          <div className="flex-1 text-sm"><span className="font-medium">{x.swimmer}</span> · <span className="text-muted-foreground">{x.t}</span></div>
          <Badge>{x.score}</Badge>
        </li>
      ))}
    </ul>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Active Tryouts" value="42" hint="Across 31 clubs" icon={Waves} accent="success" />
        <StatCard label="Pending Approvals" value="9" hint="Awaiting review" icon={Clock} accent="warning" />
      </div>
      <div className="grid gap-4 lg:grid-cols-3">
        <Card title="User Growth" action={<Badge variant="secondary"><TrendingUp className="h-3 w-3 mr-1" />+24%</Badge>}><MiniChart /></Card>
        <Card title="Club Registrations"><MiniChart /></Card>
        <Card title="Tryout Statistics"><MiniChart /></Card>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Registered Swimmers" value="312" hint="+18 this month" icon={GraduationCap} accent="success" />
        <StatCard label="Pending Registrations" value="11" hint="Awaiting confirmation" icon={Clock} accent="warning" />
      </div>
      <div className="grid gap-4 lg:grid-cols-3">
        <Card title="Club Summary">
          <dl className="space-y-3 text-sm">
            {[["Club name","Pacific Wave Aquatics"],["Founded","2018"],["Members","312 swimmers"],["Plan","Pro"]].map(([k,v]) => (
              <div key={k} className="flex justify-between"><dt className="text-muted-foreground">{k}</dt><dd className="font-medium">{v}</dd></div>
            ))}
          </dl>
        </Card>
        <Card title="Recent Coaches"><CoachList /></Card>
        <Card title="Upcoming Tryouts"><TryoutList /></Card>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <StatCard label="Upcoming Events" value="3" hint="Next: Saturday" icon={CalendarRange} accent="success" />
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Upcoming Tryouts"><TryoutList /></Card>
        <Card title="Recent Evaluations"><EvalList /></Card>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <StatCard label="Upcoming Tryouts" value="3" hint="Next: Mar 15" icon={CalendarRange} accent="aqua" />
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Child Progress">
          <div className="space-y-4">
            {[{n:"Mia Davies",l:"Level 4 — Advanced",p:78},{n:"Owen Davies",l:"Level 2 — Intermediate",p:54}].map((c) => (
              <div key={c.n}>
                ...
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-linear-to-r from-primary to-aqua rounded-full" style={{ width: `${c.p}%` }} />
                </div>
              </div>
            ))}
          </div>
        </Card>
        <Card title="Upcoming Registrations"><TryoutList /></Card>
      </div>
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  return (
    <PageShell title="Dashboard">
      ...
      {(!role || role === ("parent" as string)) && <ParentDash />}
    </PageShell>
  );
}
