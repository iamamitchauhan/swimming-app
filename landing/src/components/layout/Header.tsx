...
export function Header() {
  ...
  const initials =
    parent && `${parent.firstName[0] ?? ""}${parent.lastName[0] ?? ""}`.toUpperCase();
...
function HeaderLink({
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `rounded-md px-3 py-2 text-sm font-medium transition-colors hover:bg-muted hover:text-foreground ${isActive ? "bg-muted text-foreground" : "text-muted-foreground"}`
      }
    >
      {children}
    </NavLink>
