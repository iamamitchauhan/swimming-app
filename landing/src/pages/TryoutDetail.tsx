...
function getThemeBg(theme: string): string {
  ...
  return match ? `bg-gradient-to-br ${match.from} ${match.to}` : "bg-gradient-to-br from-indigo-700 to-slate-900";
}
...
export default function TryoutDetailPage() {
  ...
  const [selectedSlotInfo, setSelectedSlotInfo] = useState<{ date: string; time: string; label: string } | null>(null);
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className={`relative w-full overflow-hidden ${getThemeBg(tryout.purpose)}`}>
        ...
        <div className="relative mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-white/80">
            ...
            {tryout.location && (
              <span className="flex items-center gap-1.5">
                <MapPin className="h-4 w-4 text-rose-400" />{tryout.location}
              </span>
            )}
          </div>

          {tryout.description && (
            <p className="mt-2 text-sm text-white/70">{tryout.description}</p>
          )}
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4">
            ...
            <div>
              <span className="text-2xl font-extrabold text-white">{totalOpen} of {totalCap}</span>
              <div className="text-xs font-semibold uppercase tracking-wider text-white/60">
                {status === "closed" ? "Registration Closed" : "Registration Open"}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
          ...
          <div className="overflow-hidden rounded-xl border border-border">
            ...
            <div className="flex items-center justify-between bg-slate-800 px-5 py-4 text-white">
              <div>
                ...
                <p className="text-xs text-white/60 uppercase tracking-wider mt-0.5">Select your preferred arrival time</p>
              </div>
            ...
            {sessions.length > 0 ? (
                    ...
                    <div key={session.id}>
                      ...
                      {session.slots.map((sl) => {
                        ...
                        return (
                          ...
                          >
                            <span className={`w-8 shrink-0 text-xs font-bold tabular-nums ${selected ? "text-sky-700 dark:text-sky-300" : "text-muted-foreground"}`}>
                              #{sl.slotIndex + 1}
                            </span>
                            {full ? (
                              <span className="text-xs font-bold uppercase tracking-wide text-destructive">Full</span>
                            ) : selected ? (
                              <span className="text-xs font-bold uppercase tracking-wide text-sky-700 dark:text-sky-300">Selected</span>
                            ) : (
              ...
              <p className="px-5 py-8 text-center text-sm text-muted-foreground">No sessions available.</p>
            )}
          </div>
        </section>
        ...
        <section id="registration-section" className="mt-10">
          ...
          <RegistrationForm tryoutId={tryout.id} slotId={activeSlotId} sessionId={activeSessionId} selectedSlotInfo={selectedSlotInfo} segments={segments} />
        </section>
