...
export default function TryoutsPage() {
  ...
  const update = <K extends keyof TryoutFilters>(k: K, v: TryoutFilters[K]) => setFilters((f) => ({ ...f, [k]: v }));
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      ...
      {isFetching && tryouts.length === 0 ? (
        ...
        <div className="rounded-xl border border-dashed border-border p-12 text-center text-muted-foreground">No tryouts match your filters.</div>
      ) : (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{tryouts.map((t) => <TryoutCard key={t.id} tryout={t} />)}</div>
      )}
    </div>
  );
}

function FilterSelect({ placeholder, value, options, onChange }: { placeholder: string; value: string | undefined; options: string[]; onChange: (v: string | undefined) => void }) {
  return (
