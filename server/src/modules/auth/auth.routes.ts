...
const otpRateLimiter = rateLimit({
  ...
  handler: (_req, res) => sendError(res, MESSAGES.RATE_LIMIT_EXCEEDED, HTTP_STATUS.TOO_MANY_REQUESTS, "RATE_LIMIT_EXCEEDED"),
});
 ...
 * POST /verify-otp        — submit email + OTP, receive JWT (or club list)
 * POST /select-club       — select a club for multi-club users, receive new JWT
 * GET  /my-clubs          — list all clubs the user belongs to
...
authRouter.post("/select-club", authenticate, controller.selectClub);
authRouter.get("/my-clubs", authenticate, controller.listMyClubs);
