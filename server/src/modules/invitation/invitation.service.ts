...
import { BadRequestError, ForbiddenError, NotFoundError, ConflictError } from "../../shared/errors/domain.errors";
...
export class InvitationService {
  ...
  async sendInvitation(
    ...
    if (!allowed.includes(targetRole)) {
      throw new ForbiddenError(`A ${inviterRole} cannot invite someone with the role ${targetRole}.`);
    }

    const club = await ClubModel.findById(inviterClubId).lean().exec();
    if (!club) throw new NotFoundError("Club not found");
    if ((club as { status: string }).status !== "approved") {
      throw new BadRequestError("Invitations can only be sent from approved clubs.", "CLUB_NOT_APPROVED");
    }

    const existingUser = await UserModel.findOne({
      email: targetEmail,
      clubId: new mongoose.Types.ObjectId(inviterClubId),
    })
      .lean()
      .exec();
    if (existingUser) {
      throw new ConflictError("A user with this email already exists in this club.");
    }

    const existingPending = await this.repo.findPendingByEmailAndClub(targetEmail, inviterClubId);
    if (existingPending) {
    ...
    logger.info({ inviterId, targetEmail, role: targetRole, clubId: inviterClubId }, "invitation.sent");
  ...
  async acceptInvitation(
    ...
    if (!invitation) {
      throw new BadRequestError("Invalid or expired invitation link.", "INVITATION_INVALID");
    }
    ...
    const existingUser = await UserModel.findOne({
      ...
      .exec();
    if (existingUser) {
      throw new ConflictError("An account with this email already exists in this club.");
    }
    ...
    logger.info({ userId: user._id, email: invitation.email, role: invitation.role }, "invitation.accepted");
