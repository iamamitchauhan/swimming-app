...
export class SwimmerRepository {
  ...
  async findByParent(parentId: string, params: SwimmerListParams = {}): Promise<SwimmerListResult> {
    const { page = 1, limit = 10, search, isActive = true, sortBy = "createdAt", sortOrder = "desc" } = params;
    ...
    const [swimmers, total] = await Promise.all([
      SwimmerModel.find(filter).sort(sort).skip(skip).limit(limit).lean<PlainSwimmer[]>().exec(),
  ...
  async findByParentAndName(parentId: string, firstName: string, lastName: string): Promise<PlainSwimmer | null> {
    return SwimmerModel.findOne({ parentId, firstName, lastName, isActive: true }).lean<PlainSwimmer>().exec();
  }
  ...
  async update(id: string, data: Partial<Omit<PlainSwimmer, "_id" | "parentId" | "createdAt" | "updatedAt">>): Promise<PlainSwimmer | null> {
    return SwimmerModel.findByIdAndUpdate(id, { $set: data }, { new: true }).lean<PlainSwimmer>().exec();
  }
  ...
  async findByParentAndBirthDate(parentId: string, birthDate: Date): Promise<PlainSwimmer | null> {
    return SwimmerModel.findOne({ parentId, birthDate }).lean<PlainSwimmer>().exec();
  }
}
